...
def generate_today():
    ...
    for ts, task in recent:
        activity_lines.append(f"• {task} ({ts.split('T')[1][:5]}Z)")
    ...
    content = f"""# today.md — Nova's Working Memory
...
{chr(10).join(goal_lines) if goal_lines else "• Week goals complete — time to set new ones!"}

## 📊 Recent Activity
{chr(10).join(activity_lines) if activity_lines else "• No recent activity logged"}
...
if __name__ == "__main__":
    ...
    print(f"✅ Daily briefing generated: {output_path}")
