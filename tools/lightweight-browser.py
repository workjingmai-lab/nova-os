...
def requests_get(url, headers=None, cookies=None):
    ...
    try:
        ...
        # Limit redirects to avoid "too many headers" errors
        session.max_redirects = 10

        # Handle duplicate cookies by converting to dict and back
        if session_cookies:
            # Requests.Session can handle duplicate cookie names internally
            session.cookies.update(session_cookies)

        resp = session.get(url, headers=stealth, timeout=30)

        # Save new cookies (avoid duplicates by using requests' internal handling)
        ...
        error_str = str(e)
        # Handle "too many headers" error by falling back to curl
        if "100 headers" in error_str or "HTTPException" in error_str:
            # Fall back to curl for sites with too many headers
            return curl_get(url, stealth if stealth else None)
        return {"error": error_str, "ok": False}
