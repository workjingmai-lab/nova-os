...
def load_pipeline() -> List[Dict]:
    """Load revenue pipeline from revenue-tracker format."""
    if not PIPELINE_FILE.exists():
        return []
    with open(PIPELINE_FILE) as f:
        data = json.load(f)
        
    # Convert new format (grants/services/bounties) to flat list
    pipeline = []
    for category in ["grants", "services", "bounties"]:
        for item in data.get(category, []):
            pipeline.append({
                "id": f"{category}_{item['name']}",
                "company": item["name"],
                "pipeline_value": int(item["potential"]),  # Changed from "value" to "pipeline_value"
                "status": item["status"],
                "category": category[:-1] if category.endswith("s") else category,  # grants → grant
